...
function renderBody(state, now, notice) {
  const emptyReason = notice && notice.emptyReason;
  const warning = notice && notice.warning;

  if (!state || state.rows.length === 0 || !state.available) {
    let text = 'Not connected. Run <b>Claude Usage: Show Setup</b>.';
    if (emptyReason) {
      text = escapeHtml(emptyReason);
    } else if (state && !state.available) {
      text = 'Plan limits do not apply to this session (API key, Bedrock or Vertex).';
    } else if (state) {
      text = 'Waiting for the first response in a Claude Code session.';
    }
  ...
  let alert = '';
  if (warning) {
    alert = '<div class="alert">' + escapeHtml(warning) + '</div>';
  }
