...
function resetLabel(row, now) {
  ...
  // Sources disagree by under a second on the same reset, and truncating puts
  // them in different minutes. Rounding makes two rows sharing a window agree.
  const when = new Date(Math.round(row.resetsAt / 60000) * 60000);
...
function hasRows(state) {
  return Boolean(state) && state.available && state.rows.length > 0;
}

function emptyNote(state, notice) {
  if (notice.pending) {
    return 'Checking your plan limits&hellip;';
  }
  if (notice.emptyReason) {
    return escapeHtml(notice.emptyReason);
  }
  if (state && !state.available) {
    return 'Plan limits do not apply to this session (API key, Bedrock or Vertex).';
  }
  if (state) {
    return 'Waiting for the first reading.';
  }
  return 'Not connected. Run <b>Claude Usage: Show Status Line Setup</b>.';
}

function renderBody(state, now, incoming) {
  const notice = incoming || {};

  if (!hasRows(state)) {
    return '<div class="note">' + emptyNote(state, notice) + '</div>';
  }
  ...
  let alert = '';
  if (notice.warning) {
    alert = '<div class="alert">' + escapeHtml(notice.warning) + '</div>';
  }
...
// Assigning webview.html tears the document down and rebuilds it, which reads as
// a flash on every refresh. The shell is set once and its body swapped through
// postMessage instead, so only the changed text and bar widths repaint.
...
module.exports = { renderPanel, renderShell, renderBody };
