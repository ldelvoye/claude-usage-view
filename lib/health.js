...
function initialHealth() {
  return { consecutive: 0, kind: null };
}
...
function noteFailure(health, kind) {
  if (NOT_A_FAULT.has(kind)) {
    return { consecutive: health.consecutive, kind: health.kind, resting: kind };
  }
  return { consecutive: health.consecutive + 1, kind, resting: null };
}
